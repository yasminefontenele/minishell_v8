...
static void process_command(char **cmd, int index, t_shell *shell, t_list **commands_list)
{
    t_tokens *token;
    t_exec *exec_node;
    ...
    if (!exec_node || !(exec_node->args[0]) || ft_strlen(exec_node->args[0]) == 0 || ft_str_is_space(exec_node->args[0]))
    {
